...
function isFatalProductGenerationError(error) {
    ...
    return message.includes('系统维护中')
        ...
        || message.includes('邮箱池已无可用邮箱')
        || message.includes('池子里没有可用的已注册邮箱')
        || proxyFatal
...
// ─── 普号提取后台批量（三模式：rt | rc | existing_rt）───
async function startAdminFreeTokenGeneration(count, workers, mode = 'rt') {
...
// 🆕 [PATCH-Sub2API] 独立下载本批次 sub2api JSON（仅模式 B 生成）
app.get('/api/admin/products/sub2api-download/:jobKey', authenticateAdmin, async (req, res) => {
    try {
        const jobKey = String(req.params?.jobKey || '').trim();
        if (!jobKey) {
            return res.status(400).json({ success: false, message: '缺少 jobKey' });
...
app.post('/api/admin/products/generate-free', async (req, res) => {
    ...
    try {
        ...
        const launched = await startAdminFreeTokenGeneration(count, workers, 'rt');
...
// ─── 🆕 普号提取 - 三模式独立端点 ───
//   模式 A: 注册 → OAuth → RT  → product_files/refresh_tokens_register.txt
//   模式 B: 注册 → 抓 cookie    → product_files/refresh_cookies.txt
//   模式 C: 老号 → OAuth → RT   → product_files/refresh_tokens_existing.txt
function makeFreeTokenHandler(mode, modeLabel) {
    return async (req, res) => {
        ...
        try {
            ...
            const launched = await startAdminFreeTokenGeneration(count, workers, mode);
...
app.post('/api/admin/products/generate-free-rt', authenticateAdmin, makeFreeTokenHandler('rt', '普号提取-注册→RT'));
...
const GROK_JOB_TTL_MS = 30 * 60 * 1000;
function gcGrokJobs() {
    const now = Date.now();
    for (const [k, v] of grokJobs.entries()) {
        if (v.finishedAt && now - v.finishedAt > GROK_JOB_TTL_MS) grokJobs.delete(k);
    }
}

function pickGrokProxy(rawProxy) {
    const raw = String(rawProxy || '').trim();
    if (!raw) return '';
    const lines = raw.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
    if (!lines.length) return '';
    const picked = lines[Math.floor(Math.random() * lines.length)];
    if (/\{session\}/i.test(picked)) {
        const sid = `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 8)}`;
        return picked.replace(/\{session\}/gi, sid);
    }
    return picked;
}

// 列出可用银行卡（从 app_config card_pool）
app.get('/api/admin/grok/cards', async (req, res) => {
    try {
        await ensureStoreReady();
        const data = await store.getAdminData();
        const pool = Array.isArray(data?.config?.card_pool) ? data.config.card_pool : [];
        const usable = pool.filter((c) => Number(c.is_active) === 1 || c.status === 'normal');
        const masked = usable.map((c, i) => ({
            index: i,
            last4: String(c.number || '').slice(-4),
            expiry: c.expiry,
            usage_count: c.usage_count || 0,
        }));
        return res.json({ success: true, cards: masked, total: pool.length, usable: usable.length });
    } catch (error) {
        return res.status(500).json({ success: false, message: error.message });
    }
});

// 启动单次开通（后台异步执行，立即返回 jobKey）
app.post('/api/admin/grok/subscribe', async (req, res) => {
    gcGrokJobs();
    try {
        await ensureStoreReady();
        const maintenanceModeState = await store.getMaintenanceModeState();
        ...
        if (!cardNumber || !cardExp || !cvc) {
            ...
            if (Number.isInteger(idx) && idx >= 0) {
                ...
                const pool = Array.isArray(data?.config?.card_pool) ? data.config.card_pool : [];
                const usable = pool.filter((c) => Number(c.is_active) === 1 || c.status === 'normal');
                const picked = usable[idx];
                if (picked) {
                    cardNumber = cardNumber || String(picked.number || '').replace(/\s+/g, '');
                    cardExp = cardExp || String(picked.expiry || '');
                    if (!body.cvc) {
                        return res.status(400).json({ success: false, message: '卡池中卡片需手动提供 CVC' });
                    }
                }
            }
        }

        if (!sso) return res.status(400).json({ success: false, message: '缺少 sso' });
        ...
        if (!proxy) {
            try {
                const data = await store.getAdminData();
                proxy = pickGrokProxy(data?.config?.proxy || '');
            } catch (_) { /* 全局代理读取失败时仍允许直连 */ }
        } else if (/\r|\n/.test(proxy)) {
            proxy = pickGrokProxy(proxy);
        }

        const yesCaptchaKey = String(body.yesCaptchaKey || body.captchaKey || '').trim()
            || process.env.YESCAPTCHA_KEY || '';

        const jobKey = `grok-${crypto.randomUUID()}`;
        const job = {
            jobKey,
            status: 'running',
            message: '正在启动 Grok 开通流程...',
            startedAt: Date.now(),
            finishedAt: 0,
        ...
        grokJobs.set(jobKey, job);

        // 后台异步执行
        (async () => {
            const pushLog = (kind, text, extra = {}) => {
                const evt = { ts: Date.now(), kind, text, ...extra };
                job.logs.push(evt);
                if (job.logs.length > 500) job.logs.splice(0, job.logs.length - 500);
                runtimeLog.push({ jobKey, level: 'log', source: 'grok', text: `[${kind}] ${text}` });
            ...
            try {
                const result = await subscribeGrokPro({
                    sso,
                    cardNumber,
                    exp: cardExp,
                    cvc,
                    ...
                    yesCaptchaKey,
                    timeoutMs: 5 * 60 * 1000,
                    onLog: (evt) => pushLog(evt.kind || 'log', evt.text || '', evt.step ? { step: evt.step } : {}),
                });
                job.result = result;
                if (result && result.success) {
                    job.status = 'success';
                    job.message = '✅ Grok Pro 开通成功';
                    pushLog('success', job.message);
                } else {
                    job.status = 'failed';
                    job.message = `❌ ${result?.error || '未知错误'}`;
                    pushLog('error', job.message);
                }
            } catch (e) {
                job.status = 'failed';
                job.message = `❌ 异常: ${e.message}`;
...
// 查询任务快照
app.get('/api/admin/grok/jobs/:jobKey', async (req, res) => {
    gcGrokJobs();
    const job = grokJobs.get(String(req.params.jobKey || ''));
...
// 列出最近任务
app.get('/api/admin/grok/jobs', async (req, res) => {
    gcGrokJobs();
    const list = [...grokJobs.values()]
        .sort((a, b) => b.startedAt - a.startedAt)
        .slice(0, 50)
        .map((j) => ({
            jobKey: j.jobKey, status: j.status, message: j.message,
            cardLast4: j.cardLast4, startedAt: j.startedAt, finishedAt: j.finishedAt,
        }));
